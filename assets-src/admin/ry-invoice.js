...
$(function () {
    ...
    if ($('#RY_WEI_apiinfo\\[abnormal_mode\\]').length) {
        $('#RY_WEI_apiinfo\\[abnormal_mode\\]').on('change', function () {
            if ($(this).val() == 'product') {
                $('#RY_WEI_apiinfo\\[abnormal_product\\]').closest('tr').show();
            } else {
                $('#RY_WEI_apiinfo\\[abnormal_product\\]').closest('tr').hide();
            }
        }).trigger('change');
    }
