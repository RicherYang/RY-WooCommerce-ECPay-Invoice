...
final class RY_WEI_WC_Invoice_Basic
{
    ...
    public function add_invoice_info($fields)
    {
        ...
        if (did_action('woocommerce_checkout_process')) {
            $invoice_type = sanitize_locale_name($_POST['invoice_type'] ?? ''); // phpcs:ignore WordPress.Security.NonceVerification.Missing
            $invoice_carruer_type = sanitize_locale_name($_POST['invoice_carruer_type'] ?? ''); // phpcs:ignore WordPress.Security.NonceVerification.Missing
