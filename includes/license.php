...
final class RY_WEI_License extends RY_WEI_Model
{
    ...
    public static function instance(): RY_WEI_License
    {
    ...
    protected function valid_key(): void
    {
        ...
        if (!hash_equals($license_data['secret'], hash('crc32', $license_data['expire']))) {
            $this->add_valid_error_log('WEI secret check error.');
            return;
        }

        if ($license_data['expire'] <= time() - DAY_IN_SECONDS) {
            $this->add_valid_error_log('WEI expire. ' . $license_data['expire'] . ' @ ' . time());
            return;
        }

        if ($this->get_site_url() !== $license_data['url']) {
            $this->add_valid_error_log('WEI site change. ' . $license_data['url'] . ' => ' . $this->get_site_url());
            return;
        }
    ...
    {
        $json = RY_WEI_LinkServer::instance()->expire_data();
        if (is_array($json) && isset($json['data'])) {
            $this->set_license_data($json['data']);
            RY_WEI::delete_transient('expire_link_error');
        } elseif (false === $json) {
            $link_error = (int) RY_WEI::get_transient('expire_link_error');
            if ($link_error > 3) {
                $this->delete_license();
                $this->delete_license_key();
            } else {
                ...
                $link_error += 1;
                RY_WEI::set_transient('expire_link_error', $link_error);
            }
        } else {
            $this->delete_license();
            $this->delete_license_key();
    ...
    {
        $license_key = RY_WEI::get_option('license_key', '');
    ...
    public function set_license_data($data): void
    {
        ...
        RY_WEI::update_option('license_data', $data);

        RY_WEI::delete_option('pro_Data');
        RY_WEI::delete_option('pro_Key');
    }

    public function delete_license_key(): void
    {
        RY_WEI::delete_option('license_key');
        RY_WEI::delete_option('pro_Data');
        RY_WEI::delete_option('pro_Key');
    }
    ...
    {
        $license_data = RY_WEI::get_option('license_data');
        if (empty($license_data)) {
            $license_data = RY_WEI::get_option('pro_Data');
            if (!empty($license_data)) {
                unset($license_data['type']);
                $license_data['url'] = $this->get_site_url();
                $license_data = base64_encode(serialize($license_data));
                RY_WEI::update_option('license_data', $license_data);
            }
        } else {
        ...
        if (empty($license_key)) {
            $license_key = RY_WEI::get_option('pro_Key', '');
            if (!empty($license_key)) {
                RY_WEI::update_option('license_key', $license_key);
            }
        }
    ...
    private function add_valid_error_log($message)
    {
        ...
        RY_WEI::update_option('license_auto_deactivate_date', current_time('Y-m-d'));
        RY_WEI_LicenseAutoDeactivate::add_note();
    }
}
