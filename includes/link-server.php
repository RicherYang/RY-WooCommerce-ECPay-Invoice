...
final class RY_WEI_LinkServer
{
    ...
    public function activate_key()
    {
        ...
        $response = wp_remote_post($this->api_url . 'license/activate/' . $this->plugin_type, [
            ...
            'body' => wp_json_encode([
                'license_key' => RY_WEI_License::instance()->get_license_key(),
    ...
    protected function decode_response($response)
    {
        if (is_wp_error($response)) {
            RY_WEI_License::instance()->log('POST failed', WC_Log_Levels::ERROR, ['info' => $response->get_error_messages()]);
            return false;
        }

        if (wp_remote_retrieve_response_code($response) != '200') {
            RY_WEI_License::instance()->log('POST HTTP status error', WC_Log_Levels::ERROR, ['code' => wp_remote_retrieve_response_code($response)]);
            return false;
        }

        $data = @json_decode(wp_remote_retrieve_body($response), true);
        if (empty($data)) {
            RY_WEI_License::instance()->log('POST result parse failed', WC_Log_Levels::WARNING, ['data' => wp_remote_retrieve_body($response)]);
            return false;
        }
